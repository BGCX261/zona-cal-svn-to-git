...
$return = "
    ...
    // --></script>";
			...
			// drake replaced the following line
			//$return .= '<a class="'.$link_class.'" title="'.$title.'" href="#" onclick="openEventWindow('.$popup_data_index.'); return false;">';
			// this is a terrible hack. fix it with some css!
			$return .= '<span style="color:white;">';
			$popup_data_index++;
		} else {
			// drake has no idea what the following line does. do you?
			$return .= '<a class="'.$link_class.'" title="'.$title.'" href="'.$res[1].'">';
		}
		$return .= $pre_text.$event_text.$post_text.'</span>'."\n";
	}
