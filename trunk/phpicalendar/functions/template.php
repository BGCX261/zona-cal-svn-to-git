		...
		foreach ($day_array as $key) {
			...
			for ($week_loop=0; $week_loop<$week_length; $week_loop++) {
				...
				if (sizeof($event_length[$thisday]) == 0) {
					...
					$weekdisplay .= '<td id="'. $thisday . $cal_time . '" onclick="populate(\''.$thisday.$cal_time.'\')" width="' . $colspan_width . '" colspan="' . $nbrGridCols[$thisday] . '" ' . $class . '>&nbsp;</td>'."\n";
						...
						switch ($event_length[$thisday][$i]["state"]) {
							case "begin":
								...
								$event_start 	= date ($timeFormat_small, $event_start);
 								$event_calno  	= $this_time_arr[$uid]['categories'];
								...
								$colspan_width = round((80 / $nbrGridCols[$thisday]) * $drawWidth);
								$weekdisplay .= '<td id="'. $thisday . $cal_time . '" onclick="populate(\''.$thisday.$cal_time.'\');" width="'.$colspan_width.'" rowspan="' . $event_length[$thisday][$i]['length'] . '" colspan="' . $drawWidth . '" align="left" valign="top" class="eventbg2_'.$event_calno.'">'."\n";
